...
# ==============================================================================
    ...
    with tabs[1]:
        st.subheader("Subir Planilha Excel / CSV")
        arquivo = st.file_uploader("Selecione a planilha (.csv ou .xlsx)", type=["csv", "xlsx"])
        if arquivo:
            try:
                ...
                if st.button("🚀 Confirmar Importação em Massa"):
                    ...
                    for index, row in df_imp.iterrows():
                        nome_val = row.get("nome") or row.get("name") or row.get("description")
                        if pd.isna(nome_val) or not str(nome_val).strip():
                            erros_processamento.append(f"Linha {index + 1}: Nome do equipamento ausente.")
                            continue
                        nome_val = str(nome_val).strip()

                        tag_raw = row.get("tag") or row.get("code_tag")
                        if pd.notna(tag_raw) and re.match(r'^[A-Z]{4}-\d{3}$', str(tag_raw).strip().upper()):
                            tag_val = str(tag_raw).strip().upper()
                        else:
                            tag_val = f"EQP-{(index + 1):03d}"

                        marca_val = row.get("marca") or row.get("manufacturer") or "Não Informado"
                        modelo_val = row.get("modelo") or row.get("model_number") or row.get("model") or "Não Informado"
                        serial_val = row.get("serial_number") or row.get("serial") or f"SN-TEMP-{(index + 1):03d}"
                        periodo_val = row.get("periodicidade_meses") or row.get("calibration_period") or 12

                        registros_validos.append({
                            "tag": tag_val,
                            "nome": str(nome_val),
                            "marca": str(marca_val),
                            "modelo": str(modelo_val),
                            "serial_number": str(serial_val),
                            "periodicidade_meses": int(periodo_val),
                            "status": "Em Comissionamento",
                            "registrado_por": user_email
                        })

                    if erros_processamento:
                        for err in erros_processamento:
                            st.warning(f"⚠️ {err}")

                    if registros_validos:
                        supabase.table("equipamentos").upsert(registros_validos, on_conflict="tag").execute()
                        st.success(f"✅ Sucesso! {len(registros_validos)} equipamentos importados no status 'Em Comissionamento' (Estado Zero).")
                        time.sleep(2)
                        st.rerun()
                    else:
                        st.error("❌ Nenhum registro válido pôde ser extraído da planilha.")
